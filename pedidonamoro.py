...
def move_Button(event):
    x = random.randint(50, 350) # Gerar uma nova posição do x aleatroria 50 e 350
    y = random.randint(50, 150) # Gerar uma nova posição do y aleatroria 50 e 150
...
def show_response(response):
    if response == "sim":
        label_response.config(text="Você disse SIM!", fg="#FF0000")
    elif response == "nao":
        label_response.config(text="Você disse NÃO!", fg="#003CFF")
...
sim_button.configure(bg='#FF0000', fg='#FFFFFF', font=('Helvetica', 12))
nao_button.configure(bg="#003CFF", fg='#FFFFFF', font=('Helvetica', 12))
